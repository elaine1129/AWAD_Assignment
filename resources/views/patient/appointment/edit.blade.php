            ...
            <div class="col-md-12 col-lg-9 mx-auto">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Edit Appointment (ID: {{$id}})
                            <span class="badge-secondary badge">{{$status}}</span>
                        </h3>
                    </div>
                    <div class="card-body">
                        @include('partials.errors')
                        @include('partials.success')
                        <form role="form" method="POST" action="{{route('appointment.update', $id)}}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-12">
                                    <textarea rows="3" id="condition" class="form-control" name="condition">{{old('condition', $condition)}}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    ...
                                    @foreach($doctor->schedulesAvailable() as $schedule )
                                            <option value="{{$schedule->id}}" @if(old('schedule_id', $schedule_id) == $schedule->id) selected @endif>{{$schedule->date}}</option>
                                    @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                <div class="col-md-7">
                                    <button type="submit" class="btn btn-primary btn-lg">Update Appointment
                                    </button>
...
@section('script')
    <script>
        ...
        let old_schedule_id = {{$schedule_id}};
        let old_time = {{$appointment->getTimeslotIndex()}};
        $('#schedule_id').on('change', async function () {
            ...
            if(data){
                ...
                timeSelection.each(function( index ) {
                    ...
                    if($('#schedule_id').val() == old_schedule_id){
                        if(input.val() == old_time){
